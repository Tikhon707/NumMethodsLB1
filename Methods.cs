...
namespace ConsoleApp1
{
    ...
    {
        static double Divide(Func<double, double> f, double a,
            double b, double eps = 1e-10, int i = 0)
        {
            while (true)
            {
                double x0 = (a + b) / 2;

                if (f(x0) == 0.0 || Math.Abs(b - a) < eps) return x0;

                if (f(a) * f(x0) > 0) a = x0;
                else b = x0;

                Console.WriteLine(
                    "{0}ое(е) приближение - a = {1:F4}\t b = {2:F4}",
                    ++i, a, b);
            }
        }

        public static double Newton(Func<double, double> f, double a,
            double b, double eps = 1e-10)
        {
                ...
                switch (code)
                {
                    ...
                    default: 
                        throw new ArgumentException(
                            "Метод с таким кодом не существует, пожалуйста, вводите корректный код.");
                }
            }
            double result = SwitchCese(int.Parse(Console.ReadLine()));
